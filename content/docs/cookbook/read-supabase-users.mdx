...
try:
    result = vendo.data.execute("SUPABASE_BETA_RUN_SQL_QUERY", {...})
...
try {
  const result = await vendo.data.execute("SUPABASE_BETA_RUN_SQL_QUERY", { ... });
} catch (e) {
...
- Inside the customize-sandbox, `connections.get` has a 5s timeout. If your deployment has a slow connection to `VENDO_API_URL`, raise it by setting `VENDO_TIMEOUT_MS`. See [sandbox/gotchas](/docs/sandbox/gotchas).
