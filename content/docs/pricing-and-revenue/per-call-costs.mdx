...
If `unit_cost` lookup returns no active row for the provider-resolved model, the proxy returns `402` *before* forwarding upstream — no upstream call, no cost, no `usage_events` row. This is the rate-miss block: it keeps Vendo from paying upstream costs Vendo can't price back to the tenant.

## What's metered

| Integration | Metric | Pre-margin cost (illustrative) |
|---|---|---|
| OpenRouter | per 1M tokens, by model | `gpt-4o-mini`: $0.15 input / $0.60 output |
| Anthropic | per 1M tokens, by model | `claude-sonnet-4-7`: ~$3 input / $15 output |
| OpenAI | per 1M tokens, by model | varies — see OpenAI pricing |
...
| Telegram | per message sent | $0.001 (effectively rounding noise) |
| Notion | not metered through proxy | OAuth-only install, no `*-proxy` subdomain |

Provider rates change. The live, authoritative numbers are at [`vendo.run/pricing`](https://vendo.run/pricing), which reads directly from the `rates` table and serves through a 1-hour cache.
...
Hold caps are upper bounds, not averages:
...
Typical numbers (with the default 20% margin already folded in):

| Resource | Pre-margin rate | At idle (1 tenant) |
|---|---|---|
| Railway vCPU | $0.000008 / second | minimal — most idle services pin near 0% CPU |
| Railway memory | $0.000004 / GB-second | ~$10/GB-month — usually the biggest line item |
| Railway volume storage | $0.15 / GB-month | small unless you store data on disk |
| Railway egress | $0.05 / GB | usually small |
| Neon compute | $0.106 / CU-hour | ~$5/month at 0.25 CU |
| Neon storage | $0.35 / GB-month | small unless you have a heavy database |

Cloudflare Workers and Pages (the proxy, app-proxy, web app) are not metered to tenants.
