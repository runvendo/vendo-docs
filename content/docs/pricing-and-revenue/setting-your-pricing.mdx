...
`marketing` is a JSONB column on the `apps_catalog` table — the same place your tool's name, category, and marketing copy live. In your template manifest (see [Deploy & publish](/docs/deploy-and-publish/publish-to-catalog/manifest)), it's a top-level key:
...
The catalog entry is your template manifest. Phase 7 (Deploy & publish) covers the full manifest; the pricing field is a single line:

```yaml
# template.yaml in the vendo-templates repo
name: my-tool
marketing:
  pricing: credits
```

When the templates sync action runs, it upserts `apps_catalog.marketing` with this JSON. You can also flip it later — admins can update a catalog entry without a new release. But if your tool's billing surface changes (e.g. you add an integration that calls a proxy provider), update the manifest in the same PR and ship a new release.
