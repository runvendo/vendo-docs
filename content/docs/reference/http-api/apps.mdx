...
The `/api/apps/*` surface manages **apps** — the rows that back each deployment in `tool_authorizations` (formerly `apps`). One app per deployment, one or more `keys` per app, one binding per provider per app. All endpoints are session-authenticated and scoped to the caller's tenant.
...
| Field | Type | Notes |
|---|---|---|
| `id` | `string` | UUID of the `tool_authorizations` row. Same as `deployments.authorization_id`. |
