...
The transition states (`suspending`, `resuming`, `resume_failed`) exist so the dashboard can show progress and so workflows can recover from mid-transition crashes idempotently.
...
`ResumeWorkflow` is the mirror:

1. `serviceInstanceRedeploy` on each Railway service. This reuses the preserved service config + env vars — no new image fetch unless it was never deployed.
2. Unpauses the Neon branch.
