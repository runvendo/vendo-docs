...
If you're writing logic that needs `context`, check for `None` first. See [recipes/handle-not-connected](/docs/guides/recipes/handle-not-connected) for the full pattern.
...
Return this URL from your handler when `connections.get(slug)` is `None` — see [recipes/handle-not-connected](/docs/guides/recipes/handle-not-connected).
...
To read the tenant's Supabase, call `vendo.connections.get("supabase")` and use the returned `context.ref` via `vendo.data.execute("SUPABASE_BETA_RUN_SQL_QUERY", ...)`. See [recipes/read-supabase-users](/docs/guides/recipes/read-supabase-users).
