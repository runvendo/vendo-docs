...
- A Vendo account with the integration you want to add already enabled in the platform — check **Settings → Connections** in the dashboard. Notion, Google Drive, Slack, Linear, Stripe, GitHub, and ~15 other Composio-brokered providers are wired today. If the provider doesn't exist yet, the runtime SDK can't call it; [open an issue](https://github.com/runvendo/vendo/issues/new) describing the auth model and what the tool needs.
...
1. **Set `VENDO_API_KEY`** in your local env. Use a personal `vendo_sk_*` from the dashboard's **Settings → API Keys** page — that's your own developer key, fine to use locally.
...
- **From the dashboard.** Open **Settings → Connections**, click **Connect** next to Notion, and complete the OAuth popup. Re-run the script; it now succeeds.
...
Once the user finishes OAuth, the popup closes itself. Vendo emits a `connection.connected` webhook to your deployment; your handler calls `vendo.invalidate("notion")` to clear the in-process token cache, and the next call to `list_notion_databases_safe` succeeds.

The Webhook side of this loop is the [Verify a Webhook recipe](/docs/guides/recipes/verify-webhook); the equivalent JSON contract for `data.execute`-style code paths is in [Handle NotConnected](/docs/guides/recipes/handle-not-connected). `NeedsReauth` is the same shape but raised when an existing connection's token was revoked upstream — the user has to redo OAuth from scratch.
...
3. They click **Connect**. A popup opens at `https://vendo.run/api/oauth/notion/start` (or `composio` for Composio-brokered providers), then to Notion's hosted OAuth page where they pick the workspaces they want to share and click **Allow**.
...
- **Webhook signal.** Vendo emits `connection.disconnected` and then `connection.connected` events. Your webhook handler should call `vendo.invalidate(slug)` to clear the SDK's in-process token cache so the next read pulls the fresh credential without waiting for a restart.

A minimal webhook handler that does both:
...
def handle_vendo_webhook(headers: dict, body: bytes) -> dict:
    try:
        event = vendo.webhooks.verify(headers, body)
    except ValidationError:
        return {"status": 400, "error": "invalid signature"}

    if event.type in ("connection.connected", "connection.disconnected"):
...
- **Webhooks from the provider.** Some integrations (Telegram, Stripe) deliver inbound updates Vendo routes through `hooks.vendo.run`. Different shape — covered separately in [Webhooks](/docs/concepts/webhooks) and the [Verify a webhook recipe](/docs/guides/recipes/verify-webhook).
