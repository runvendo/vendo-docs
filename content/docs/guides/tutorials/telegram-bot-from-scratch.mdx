...
1. Log into [vendo.run](https://vendo.run) and open **Connections** in the dashboard sidebar (`/connections`).
...
2. **Verify the webhook with Telegram** — `curl "https://api.telegram.org/bot<TOKEN>/getWebhookInfo"` should return `url: "https://<tenant-slug>-<deployment-slug>.vendo.run/telegram/webhook"` with `has_custom_certificate: false` and `pending_update_count: 0`. If `last_error_message` is set, it's almost always a 403 from the handler — the `secret_token` you set doesn't match what your container sees.
