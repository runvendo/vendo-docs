...
{/* TODO(security): confirm `security@vendo.run` mailbox exists and is monitored before publishing — the address is not referenced anywhere in the monorepo. If it doesn't exist, replace below with a GitHub Security Advisory link on the appropriate vendo repo, or another verified channel. */}
