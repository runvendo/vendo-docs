...
```
vendo_sk_<random-32-char-base62>
```

The plaintext is returned exactly once — when the key is first issued — and is never persisted in cleartext anywhere in Vendo's infrastructure. What is stored is the SHA-256 hash of the plaintext. At request time, the proxy hashes the bearer it received and looks the hash up to find the key's metadata. A stolen key cannot be re-derived from any database row.

There is no way to retrieve a key after issuance. Lost keys are revoked and replaced; not recovered.
