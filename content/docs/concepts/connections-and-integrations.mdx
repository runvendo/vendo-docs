...
| `user_oauth` | Tenant ran an OAuth flow against the upstream provider (their own user account) |
| `oauth_app_install` | Tenant installed Vendo's OAuth app at the upstream (workspace-level grant) |
| `webhook_inbound` | Provider pushes events into Vendo via a webhook URL the tenant configured |
