...
- `margin_pct` is Vendo's markup. The current global default is 20%, applied to every call. Individual integrations can override; some tenants have negotiated rates.
...
## Suspension is gentle

When a tenant runs out of credits, their deployment goes through a state machine: `running → suspending → suspended → resuming → running`. The container is stopped, the database is paused (Neon's branch-pause feature), the public URL serves a status page. Everything is preserved.
