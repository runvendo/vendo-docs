...
- **Neon Postgres**: `aws-us-east-1` by default. A template manifest can override per-database via the schema's `databases[].region` field, but most tools take the default.
- **Railway compute**: Vendo doesn't pass an explicit region to Railway today, so projects inherit whatever region Railway picks for the workspace. In practice this is US-aligned, but it's not guaranteed in code.
